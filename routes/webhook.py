...
async def tradingview_webhook_route(data: TradingViewWebhookModel, request: Request, background_task: BackgroundTasks):
    ...
    if memory.get_trading_view_hooked_trade_side(data.epic, data.hook_name, data.trade_mode) == data.direction: # check if the trade is already executed on trade side
