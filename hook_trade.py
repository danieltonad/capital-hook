...
from database import save_trade_history, save_position
...
class HookedTradeExecution:
    ...
    def __init__(self, trade_direction: TradeDirection, epic: str, trade_amount: int, profit: int, loss: int, hook_name: str, exit_criteria: List[ExitType], trail_sl: int, trade_mode: TradeMode):
    ...
    async def execute_trade(self):
        try:
            ...
            # open position
            self.deal_id, open_price = await open_trade(epic=self.epic, size=self.trade_size, trade_direction=self.trade_direction, trade_mode=self.position_mode)
            if not self.deal_id:
